...
- `gemini-designer advise` — Use for existing HTML/CSS/TSX or design files. Requires `-f` or `-i`, and requires a readable markdown path with `-o`. Gemini gives concrete implementation-oriented visual suggestions, reuse reminders, and pseudo-code snippets when useful. Use this for small refinements and project-style consistency.
- `gemini-designer direction` — Use before implementation when the task needs a stronger idea, art direction, visual metaphor, design imagery markdown, or high-level design direction. Files are optional. Always provide a readable markdown path with `-o`.
...
For `advise` and `direction`, always name the markdown file at call time with `-o`. Use `.gemini-designer/` and a readable filename that explains the artifact, such as `.gemini-designer/accounts-filter-advice.md`, `.gemini-designer/museon-home-art-direction.md`, or `.gemini-designer/pricing-page-design-imagery.md`.
...
gemini-designer advise "判断这个页面是否符合项目现有视觉风格，并给出具体优化建议" \
  ...
  -o ./.gemini-designer/design-page-advice.md
```
...
gemini-designer advise "结合截图判断这个页面哪里最影响视觉完成度" \
  ...
  -o ./.gemini-designer/current-page-screenshot-advice.md

gemini-designer direction "参考这张图，提炼更适合产品的设计意象 markdown" \
  -i ./references/moodboard.png \
  -o ./.gemini-designer/product-design-imagery.md
```
...
```bash
gemini-designer advise "给这个设计规范页提视觉优化建议" -f ./design.html -o ./.gemini-designer/design-page-advice.md
gemini-designer advise "结合截图给这个页面提视觉优化建议" -f ./design.html -i ./screenshots/current.png -o ./.gemini-designer/page-screenshot-advice.md
gemini-designer direction "给这个产品生成设计意象 markdown" -o ./.gemini-designer/product-design-imagery.md
gemini-designer direction "基于这个页面提炼更强的视觉意象" -f ./design.html -o ./.gemini-designer/page-design-imagery.md
...
gemini-designer direction \
  ...
  -o ./.gemini-designer/product-art-direction.md
```
